...
class ChatToolRequest(BaseModel):
    ...
    @field_validator("uid")
    @classmethod
    def uid_must_be_non_empty(cls, v: Optional[str]) -> str:
        v = (v or "").strip()
        if not v:
...
class SearchDropboxRequest(ChatToolRequest):
    ...
    @classmethod
    def query_must_be_valid(cls, v: Optional[str]) -> str:
        v = (v or "").strip()
        return v
...
class ReadDropboxFileRequest(ChatToolRequest):
    ...
    path: Optional[str] = Field(None, description="Full file path in Dropbox")

    @field_validator("path")
    @classmethod
    def path_must_be_non_empty(cls, v: Optional[str]) -> str:
        v = (v or "").strip()
        if not v:
